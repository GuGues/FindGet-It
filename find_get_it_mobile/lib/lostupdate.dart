...
class LostUpdatePage extends StatefulWidget {
  final String lostIdx;
  const LostUpdatePage({Key? key, required this.lostIdx}) : super(key: key);

  @override
  _LostUpdatePageState createState() => _LostUpdatePageState();
}
...
Future<Member> fetchItems() async {
  final String url =
      appConfig.url + '/app/lostUpdate/' + GlobalState.loginEmail ;
...
class _LostUpdatePageState extends State<LostUpdatePage> {

   fetchLost(lostIdx) async{
    final String url =
        appConfig.url + '/app/lostUpdate/get-lost/' + lostIdx  ;
    final response = await http.get(Uri.parse(url));

    if (response.statusCode == 200) {
      // 서버에서 받은 JSON 데이터를 파싱하여 Member 객체로 변환
      Map<String, dynamic> data = json.decode(utf8.decode(response.bodyBytes));
      if (data['LOSTIDX'] == null) {
        throw Exception('Invalid JSON: LostItem is missing or malformed');
      }
      print(data);
      LostItem item = LostItem.fromJson(data);
      setState(() {
        lost_titleController = TextEditingController(text:item.lostTitle);
        lost_contentController = TextEditingController(text:item.lostContent);
        l_location_detailController = TextEditingController(text:item.lLocationDetail);
        rewardController = TextEditingController(text:item.reward.toString());
        l_item_detailController = TextEditingController(text:item.lItemDetail);
        ...
        lost_date =DateTime.parse( item.lostDate );
      });
    } else {
      throw Exception('Failed to load member: ${response.statusCode}');
    }
  ...
  updateLost() async {
    ...
    try {
      ...
      var response = await dio.post(
        appConfig.url + '/app/lostUpdate/update',
        data: formData,
      );
      print('성공적으로 업로드했습니다');
...
/*    var uri = Uri.parse(appConfig.url + '/app/lostUpdate/insert-post');
