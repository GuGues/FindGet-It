...
import 'package:find_get_it_mobile/models.dart';
...
class FoundUpdatePage extends StatefulWidget {
  final String foundIdx;
  const FoundUpdatePage({Key? key, required this.foundIdx}) : super(key: key);

  @override
  _FoundUpdatePageState createState() => _FoundUpdatePageState();
}
...
Future<Member> fetchItems() async {
  final String url =
      appConfig.url + '/app/foundUpdate/' + GlobalState.loginEmail ;
...
class _FoundUpdatePageState extends State<FoundUpdatePage> {

   fetchFound(foundIdx) async{
    final String url =
        appConfig.url + '/app/foundUpdate/get-found/' + foundIdx  ;
    final response = await http.get(Uri.parse(url));

    if (response.statusCode == 200) {
      // 서버에서 받은 JSON 데이터를 파싱하여 Member 객체로 변환
      Map<String, dynamic> data = json.decode(utf8.decode(response.bodyBytes));
      if (data['FOUNDIDX'] == null) {
        throw Exception('Invalid JSON: FoundItem is missing or malformed');
      }
      print(data);
      FoundItem item = FoundItem.fromJson(data);
      setState(() {
        found_titleController = TextEditingController(text:item.foundTitle);
        found_contentController = TextEditingController(text:item.foundContent);
        f_location_detailController = TextEditingController(text:item.fLocationDetail);
        f_item_detailController = TextEditingController(text:item.fItemDetail);
        selectedCategory = item.itemName;
        selectedLocation = item.sidoName+" "+item.gugunName;
        selectedColor = item.colorName;
        item_code = int.parse(item.itemCode);
        location_code = int.parse(item.locationCode);
        color_code = int.parse(item.colorCode);
        found_date =DateTime.parse( item.foundDate );
      });
    } else {
      throw Exception('Failed to load member: ${response.statusCode}');
    }
  ...
  updateFound() async {
    ...
    try {
      ...
      var response = await dio.post(
        appConfig.url + '/app/foundUpdate/update',
        data: formData,
      );
      print('성공적으로 업로드했습니다');
...
/*    var uri = Uri.parse(appConfig.url + '/app/foundUpdate/insert-post');
    final response = await http.post(uri,
        ...
        body: jsonEncode(<String, String>{
          ...
          'found_title': found_titleController.text,
          'found_content': found_contentController.text,
  ...
  TextEditingController f_item_detailController = TextEditingController();
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      TextFormField(
                        controller: f_item_detailController,
                        maxLines: 4,
                        decoration: InputDecoration(
                          labelText: '내용',
                          border: OutlineInputBorder(),
